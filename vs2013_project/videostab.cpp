...
void run()                       //show the original and stabilized image
{
    VideoWriter writer;
    Mat stabilizedFrame;
    int nframes = 0;
	...
	while ((!(stabilizedFrame = stabilizedFrames->nextFrame()).empty()) && !(originalImage = source_all->nextFrame()).empty())
    {
#ifdef TimeDebug
		...
		t = getTickCount();
#endif // TimeDebug
		imshow("stabilized_image", stabilizedFrame);      //show the images afer stabilization
		imshow("original_image", originalImage);      //show the origin image
		char c;
		if ((c = waitKey(20)) == 27)                //use "ESC" to break
		{
			break;
		}
